...
# running instance into reporting/assets/ as a reviewable YAML tree.
#
# This refreshes the git source of truth. After running, regenerate the Helm
# values with build-values-assets.sh so the bootstrap import picks up changes.
#
# Usage:
#   reporting/scripts/export-assets.sh
#
# Env overrides:
...
rm -rf "${ASSETS_DIR}"
mkdir -p "${ASSETS_DIR}"
# copy tree, stripping Superset's "_<id>" filename suffix for stable names
cp "${root}/metadata.yaml" "${ASSETS_DIR}/metadata.yaml"
while IFS= read -r -d '' f; do
  rel="${f#"${root}/"}"
  dir="$(dirname "$rel")"
  base="$(basename "$rel" .yaml)"
  base="${base%_[0-9]*}"          # drop trailing _<id>
...
echo
echo "Next: reporting/scripts/build-values-assets.sh"